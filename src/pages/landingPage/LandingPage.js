...
const LandingPage = () => {
  ...
  const [fieldError, setFieldError] = useState({
    ...
    number: { message: "", error: false },
  });
  const validateValues = () => {
    if (values.email && values.name_ && values.number) {
      if (
  ...
  const submit = async () => {   
   const createDetails = {
      ...
      eventName: values.name_,
      numberOfParticipants: values.number,
    };
    ...
    let url =
      "https://secretesant-a.herokuapp.com/api/v1/secretSanta/createEvent";
    try {
  ...
  return (
    <div>
      <AccessCodeModal
        accessOpen={AccessCodeEmailOpen}
        closeAccess={closeAccessModal}
      />
      <EventCreatedModal
        modalOpen={eventCreatedModalOpen}
        ...
        {landing_Page ? (
          <div className="firstSection">
            <div className="text">
              ...
              <p>
                Start your anonymous matching with family, friends and
                colleagues! Happy gifting........
              </p>
            </div>
            <img src={illustration} width={300} alt="img" />
            <Button
              onClick={submitAccessID}
              type={"button"}
              buttonStyle={"solid"}
            >
              Access My Event
